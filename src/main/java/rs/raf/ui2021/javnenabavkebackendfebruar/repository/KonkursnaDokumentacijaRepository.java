package rs.raf.ui2021.javnenabavkebackendfebruar.repository;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;
import rs.raf.ui2021.javnenabavkebackendfebruar.model.JavnaNabavka;
import rs.raf.ui2021.javnenabavkebackendfebruar.model.KonkursnaDokumentacija;

import java.util.Optional;

@Repository
public interface KonkursnaDokumentacijaRepository extends JpaRepository<KonkursnaDokumentacija, Long> {

    Optional<KonkursnaDokumentacija> findByJavnaNabavka(JavnaNabavka javnaNabavka);
}